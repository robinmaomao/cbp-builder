...
- 检测到有效 `.cbp` 工程时，尝试在当前工作区禁用 `C/C++ Compile Run` 扩展，避免编译入口冲突。
...
4. 在状态栏点击 **增量编译** 或 **全量编译**，也可以打开命令菜单执行对应命令。
5. 在工程节点、编辑器标签页或工作区任意文件上右键选择增量编译或全量编译。
6. 在“输出”面板选择 `CBP Builder` 查看日志。

检测到 `.cbp` 工程后，扩展会尝试在当前工作区禁用 `C/C++ Compile Run`。如果 VS Code 无法执行自动禁用，请在扩展面板中对该扩展选择“在工作区禁用”。
...
详细变更请参阅 [CHANGELOG.md](CHANGELOG.md)。当前版本为 `v0.0.16-beta`。
